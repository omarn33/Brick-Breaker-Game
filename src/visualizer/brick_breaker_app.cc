...
void BrickBreakerApp::draw() {
    ...
    if (new_game_) {
        ...
        ci::gl::drawStringCentered(
                "BrickBreaker",
                glm::vec2(kWindowWidth / 2, kWindowHeight / 4), ci::Color("white"),
                ci::Font("Century", 300.0f));

        ci::gl::drawStringCentered(
                "Press [S] to Start Game",
                glm::vec2(kWindowWidth / 2, kWindowHeight / 2), ci::Color("white"),
                ci::Font("Arial", 150.0f));
        ...
        if (generate_new_level_) {
            current_level_ = GenerateLevel(current_level_.GetLevelNumber());
...
void BrickBreakerApp::update() {
    if (!has_game_ended_) {
        ...
        // Determine if the ball collided with the wall
...
void BrickBreakerApp::mouseMove(ci::app::MouseEvent event) {
    if ((event.getPos().x - (kPaddleWidth / 2) >= container_top_left_corner_.x + (kContainerWallStroke / 2)) &&
        event.getPos().x + (kPaddleWidth / 2) <=
        container_bottom_right_corner_.x - (kContainerWallStroke / 2)) {
        paddle_.SetPaddlePosition(
                glm::vec2{event.getPos().x - (kPaddleWidth / 2), paddle_.GetPaddlePosition().y});
    }
}

Level BrickBreakerApp::GenerateLevel(size_t level) {
...
void BrickBreakerApp::DisplayScoreboard() {
    // Draw Scoreboard
    ci::gl::drawStringCentered(
            std::to_string(score_),
            glm::vec2(kScoreboardTextX, kScoreboardTextY), ci::Color("Black"),
            ci::Font("Impact", 200.0f));

    ci::gl::drawStringCentered(
            std::to_string(ammo_),
            glm::vec2(kScoreboardTextX, 2 * kScoreboardTextY), ci::Color("Black"),
            ci::Font("Impact", 200.0f));

    ci::gl::drawStringCentered(
            std::to_string(lives_),
            glm::vec2(kScoreboardTextX, 3 * kScoreboardTextY), ci::Color("Black"),
            ci::Font("Impact", 200.0f));

    ci::gl::drawStringCentered(
            std::to_string(high_score_),
            glm::vec2(kScoreboardTextX, 4 * kScoreboardTextY), ci::Color("Black"),
            ci::Font("Impact", 200.0f));
}

void BrickBreakerApp::DrawDefaultStage() {
    // Set ball position to the paddle position
    ball_.SetPosition(glm::vec2{paddle_.GetPaddlePosition().x + (kPaddleWidth / 2),
                                paddle_.GetPaddlePosition().y - kBallRadius});
    ...
    ci::gl::drawStringCentered(
            ...
            glm::vec2(x_1 + (width / 2), y_1 + (length / 10)), ci::Color("yellow"),
            ci::Font("Impact", 250.0f));

    // Draw message
    ci::gl::drawStringCentered(
            "Lives: " + std::to_string(lives_),
            glm::vec2(x_1 + (width / 2), y_1 + (length / 2)), ci::Color("yellow"),
            ci::Font("Impact", 150.0f));

    // Draw message
    ci::gl::drawStringCentered(
            "Press [Space] to Continue",
            glm::vec2(x_1 + (width / 2), y_1 + (length * 3 / 4)), ci::Color("yellow"),
            ci::Font("Impact", 120.0f));

}

void BrickBreakerApp::DisplayWinScreen() {
    // Set ball position to the paddle position
    ball_.SetPosition(glm::vec2{paddle_.GetPaddlePosition().x + (kPaddleWidth / 2),
                                paddle_.GetPaddlePosition().y - kBallRadius});
    ...
    ci::gl::drawStringCentered(
            ...
            glm::vec2(x_1 + (width / 2), y_1 + (length * 4 / 5)), ci::Color("yellow"),
            ci::Font("Impact", 150.0f));
}

void BrickBreakerApp::DisplayGameOverScreen() {
    // Set ball position to the paddle position
    ball_.SetPosition(glm::vec2{paddle_.GetPaddlePosition().x + (kPaddleWidth / 2),
                                paddle_.GetPaddlePosition().y - kBallRadius});
    ...
    ci::gl::drawStringCentered(
            "GAME OVER!",
            glm::vec2(x_1 + (width / 2), y_1 + (length / 10)), ci::Color("red"),
            ci::Font("Impact", 250.0f));
